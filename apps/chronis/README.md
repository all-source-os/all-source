...
| `cn task reparent <id> <parent>` | | Move a task under a new parent (`--root` detaches, `--force` clears warnings) |
...
| `task.updated` | `cn task edit`, `cn task reparent` (a present-and-null `parent` clears it) |
